/*
 * File:   swarm_bots_main_ver3.c
 * Author: Lim
 *
 * Created on June 21, 2019, 8:42 PM
 */
    ...
    if(_T1IF)
    {
        ...
        switch(currentState){
            ...
            case ADJUST_RG:
                ...
                {
                    if(maxGreen[0] == 2 && (maxGreen[1]*10 / rgb[maxGreen[0]][3]) >= SENSITIVITY) currentState = ADJUST_LG;
                    else if(maxGreen[0] == 1 && abs(maxGreen[1]-lastF) <= ERR_RANGE) 
                    {
            ...
            case ADJUST_LG:
                if((maxGreen[1] >= maxRed[1]))
                {
                    if(maxGreen[0] == 0 && (maxGreen[1]*10 / rgb[maxGreen[0]][3]) >= SENSITIVITY) currentState = ADJUST_RG;
                    else if(maxGreen[0] == 1 && abs(maxGreen[1] - lastF) <= ERR_RANGE) 
                    {
                        currentState = DEFAULT_STATE;
                        lastF = 0;
            ...
            case ADJUST_RR:
                ...
                {
                    if(maxRed[0] == 2 && (maxRed[1]*10 / rgb[maxRed[0]][3]) >= SENSITIVITY) currentState = ADJUST_LR;
                    else if(maxRed[0] == 1 && abs(maxRed[1] - lastF) <= ERR_RANGE) 
                    {
                        currentState = DEFAULT_STATE;
                        lastF = 0;
                        break;
            ...
            case ADJUST_LR:
                if((maxRed[1] >= maxGreen[1]))
                {
                    if(maxRed[0] == 0 && (maxRed[1]*10 / rgb[maxRed[0]][3]) >= SENSITIVITY) currentState = ADJUST_RR;
                    else if(maxRed[0] == 1 && abs(maxRed[1] - lastF) <= ERR_RANGE) 
                    {
                        currentState = DEFAULT_STATE;
                        lastF = 0;
