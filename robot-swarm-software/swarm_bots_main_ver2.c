/*
 * File:   swarm_bots_main_ver2.c
 * Author: Lim
 *
 * Created on June 9, 2019, 2:53 PM
 */
...
{
    if(currentState == ADJUST_RG || currentState == ADJUST_RR) turnRight(70);
    else if(currentState == ADJUST_LG || currentState == ADJUST_LR) turnLeft(70);
    ...
    if(_T1IF)
    {
        ...
        switch(currentState){
            ...
            case ADJUST_RG:
                ...
                {
                    if(maxGreen[0] == 2) currentState = ADJUST_LG;
                    else if(maxGreen[0] == 1 && abs(maxGreen[1]-lastF) <= ERR_RANGE) currentState = DEFAULT_STATE;
                ...
                else currentState = DEFAULT_STATE;          //In the case where all light sources are off, assume default state
                break;
            case ADJUST_LG:
                if((maxGreen[1] >= maxRed[1]) && ((maxGreen[1]*10 / rgb[maxGreen[0]][3]) >= SENSITIVITY))
                {
                    if(maxGreen[0] == 0) currentState = ADJUST_RG;
                    else if(maxGreen[0] == 1 && abs(maxGreen[1] - lastF) <= ERR_RANGE) currentState = DEFAULT_STATE;
                ...
                break;
            case ADJUST_RR:
                if((maxRed[1] >= maxGreen[1]) && ((maxRed[1]*10 / rgb[maxRed[0]][3]) >= SENSITIVITY))
                {
                    if(maxRed[0] == 2) currentState = ADJUST_LR;
                    else if(maxRed[0] == 1 && abs(maxRed[1] - lastF) <= ERR_RANGE) currentState = DEFAULT_STATE;
            ...
            case ADJUST_LR:
                if((maxRed[1] >= maxGreen[1]) && ((maxRed[1]*10 / rgb[maxRed[0]][3]) >= SENSITIVITY))
                {
                    if(maxRed[0] == 0) currentState = ADJUST_RR;
                    else if(maxRed[0] == 1 && abs(maxRed[1] - lastF) <= ERR_RANGE) currentState = DEFAULT_STATE;
